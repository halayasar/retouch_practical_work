...
def iou_score(gt, pr, class_weights=1., class_indexes=None, smooth=SMOOTH, per_image=False, threshold=None, **kwargs):
    ...
    # score calculation
    CROP_SHAPE = KB.int_shape(pr)

    y_true_ = tf.compat.v1.image.resize_image_with_crop_or_pad(gt, target_height=CROP_SHAPE[1], target_width=CROP_SHAPE[2])
    y_true_ = tf.cast(y_true_, tf.float32)

    y_pred_ = KB.clip(pr, KB.epsilon(), 1. - KB.epsilon())
